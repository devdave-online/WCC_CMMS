    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($new_status === 'Completed' && !empty($checklist_data) && !empty($wo['started_at'])) {
            ...
            if ($elapsed_secs < $expected_secs) {
                ...
                echo "<h2 style='color: #f87171; margin-top: 0; font-family: sans-serif;'>" . htmlspecialchars(__('wo.qc_alert'), ENT_QUOTES, 'UTF-8') . "</h2>";
                echo "<p style='color: #e2e8f0; font-size: 1.1em; line-height: 1.5; font-family: sans-serif;'>" . htmlspecialchars(__('wo.qc_too_fast', ['actual' => (string)round($elapsed_secs/60), 'expected' => (string)$total_expected_mins]), ENT_QUOTES, 'UTF-8') . "</p>";
                echo "<a href='/_maint/wo_takeover.php?wo_id=$wo_id' style='background: #ef4444; color: white; padding: 12px 24px; border-radius: 8px; margin-top: 20px; display: inline-block; text-decoration: none; font-weight: bold; font-family: sans-serif; transition: background 0.2s;' onmouseover='this.style.background=\"#dc2626\"' onmouseout='this.style.background=\"#ef4444\"'>← " . htmlspecialchars(__('common.back'), ENT_QUOTES, 'UTF-8') . "</a>";
                echo "</div></body></html>";
                exit;
            }
        }
        ...
        if ($new_status === 'Completed') {
            ...
            wcc_notify_perms(['view_work_orders', 'view_statistics'], 'wo_completed', $msg, $link, 'success', $actor);
        }
...
?>
<?php
$page_title = __('wo.takeover_title') . ' #' . $wo_id;
...
<div class="form-container">
    <div class="page-header">
        <h1>🛠️ <?= __e('wo.takeover_title') ?>: #<?= $wo_id ?></h1>
        <a href="/_maint/pm_calendar.php" class="nav-btn">📅 <?= __e('wo.calendar') ?></a>
    </div>
    ...
    <div style="background: rgba(0,0,0,0.2); padding: 15px; border-radius: 8px; border: 1px solid var(--panel-border); margin-bottom: 20px;">
        ...
        <p style="color: var(--text-secondary); margin-bottom: 5px;"><strong><?= __e('th.scheduled_date') ?>:</strong> <?= htmlspecialchars($wo['scheduled_date']) ?></p>
        <p style="color: var(--text-secondary); margin-bottom: 5px;"><strong><?= __e('common.equipment') ?>:</strong> <?= htmlspecialchars($wo['equipment_name'] ?? __('common.na')) ?></p>
        <p style="color: var(--text-secondary); margin-bottom: 5px;"><strong><?= __e('th.assigned_to_cap') ?>:</strong> <?= htmlspecialchars($wo['assigned_user'] ?? __('common.na')) ?></p>
        <p style="color: var(--text-primary); margin-top: 15px;"><strong><?= __e('wo.instructions_label') ?></strong><br><?= nl2br(htmlspecialchars($wo['description'])) ?></p>
    </div>
    ...
    <form method="POST" id="takeoverForm" enctype="multipart/form-data" onsubmit="return validateTakeoverForm(this)">
        <?php if ($wo['status'] === 'Scheduled' && empty($wo['started_at'])): ?>
            ...
            <div style="text-align: center; margin: 40px 0;">
                ...
                <p style="color: var(--text-secondary); margin-top: 15px;"><?= __e('wo.start_work_hint') ?></p>
            </div>
        <?php else: ?>
        ...
        <?php if (!empty($checklist_data)): ?>
            <div style="background: var(--success-bg); border: 1px solid var(--success-border); padding: 15px; border-radius: var(--radius-sm); margin-bottom: 20px;">
                <h3 style="color: var(--success); margin-top: 0;">✅ <?= __e('wo.pm_checklist_expected', ['mins' => (string)$total_expected_mins]) ?></h3>
                <?php if (!empty($wo['started_at'])): ?>
                    <p style="color:var(--text-secondary); font-size:0.9em; margin-bottom:15px;"><?= __e('wo.started_at') ?> <strong><?= htmlspecialchars($wo['started_at']) ?></strong></p>
                <?php endif; ?>
                    ...
                    <?php foreach($checklist_data as $idx => $item): ?>
                        <div style="position: relative; background: rgba(0,0,0,0.25); padding: 14px 18px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.05); margin-bottom: 5px; display: flex; flex-direction: column; gap: 8px;">
                            ...
                            <div style="display: flex; justify-content: flex-start;">
                                <span style="font-size: var(--fs-xs); color: var(--text-muted); background: rgba(0,0,0,0.4); padding: 3px 8px; border-radius: var(--radius-sm); border: 1px solid var(--panel-border);">⏱️ <?= (int)$item['expected_time_mins'] ?> <?= __e('common.mins') ?></span>
                            </div>
                                ...
                                <div style="display: flex; align-items: center; justify-content: flex-end; gap: 20px; margin-left: 20px;">
                                    ...
                                    <?php if (!$isLocked): ?>
                                        <div style="display: flex; align-items: center; gap: 10px;">
                                            ...
                                            <label style="cursor: pointer; font-size: 1.8em; line-height: 1; margin: 0; opacity: 0.7; transition: opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.7'" title="<?= __e('wo.upload_photo_title') ?>">
                                                📸
        ...
        <label for="wo_tech_name"><?= __e('wo.tech_taking_over') ?></label>
        <input type="text" id="wo_tech_name" value="<?= htmlspecialchars($_SESSION['username'] ?? __('common.unknown')) ?>" readonly style="font-weight: bold;">

        <?php if (!empty($required_parts)): ?>
            <label><?= __e('wo.required_parts_consume') ?></label>
            <div style="background: rgba(0,0,0,0.1); padding: 15px; border-radius: 8px; border: 1px solid var(--panel-border); margin-bottom: 15px;">
                <?php foreach($required_parts as $p): ?>
                    <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text-primary); margin: 5px 0;">
                        ...
                        <?= htmlspecialchars($p['part_name'] . ' [' . $p['internal_code'] . ']') ?> <?= __e('wo.deduct_1x') ?>
                    </label>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
        ...
        <label for="part_search"><?= __e('wo.parts_consumed') ?> <span style="color:var(--text-secondary); font-weight:normal; font-size:0.85em;"><?= __e('wo.parts_consumed_hint') ?></span></label>
        <div class="searchbox-container" style="position:relative;">
            <input type="text" id="part_search" placeholder="<?= __e('wo.part_search') ?>" autocomplete="off" 
        ...
        <label for="wo_action_taken"><?= __e('wo.action_taken') ?></label>
        <textarea name="action_taken" id="wo_action_taken" rows="3" required placeholder="<?= __e('wo.action_notes') ?>" <?= $isLocked ? 'disabled' : '' ?>></textarea>

        <label for="wo_status"><?= __e('wo.final_status') ?></label>
        <select name="status" id="wo_status" <?= $isLocked ? 'disabled' : '' ?>>
            <?php /* option values are stored DB enums — do not translate values */ ?>
            <option value="Completed" <?= $wo['status'] === 'Completed' ? 'selected' : '' ?>>✅ <?= __e('wo.status_completed') ?></option>
            <option value="In Progress" <?= $wo['status'] === 'In Progress' ? 'selected' : '' ?>>⏳ <?= __e('wo.status_in_progress') ?></option>
            <option value="Missed" <?= $wo['status'] === 'Missed' ? 'selected' : '' ?>>⚠️ <?= __e('wo.status_missed') ?></option>
            <option value="Cancelled" <?= $wo['status'] === 'Cancelled' ? 'selected' : '' ?>>❌ <?= __e('wo.status_cancelled') ?></option>
        </select>
        
        <?php if ($isLocked): ?>
            <button type="button" class="btn btn-block" style="margin-top: 15px;" disabled><?= __e('wo.locked_status', ['status' => $wo['status']]) ?></button>
        <?php else: ?>
            <button type="submit" class="btn btn-success btn-block" style="margin-top: 15px;"><?= __e('wo.log_close') ?></button>
        <?php endif; ?>
        <?php endif; ?>
    </form>
</div>
    ...
    selectedUsedParts.forEach((part, idx) => {
        ...
        row.innerHTML =
            '<div class="wcc-part-info">' +
                ...
                '<div class="wcc-part-stock ' + (warn ? 'warn' : '') + '">' + (typeof t === 'function' ? t('wo.in_stock_line', {stock: stock, after: after}) : ('In stock: ' + stock + ' → ' + after + ' after')) + (warn ? ' &nbsp;⚠ ' + (typeof t === 'function' ? t('wo.below_minimum') : 'below minimum') : '') + '</div>' +
            '</div>' +
            '<div class="wcc-qty-stepper">' +
                '<button type="button" class="qty-btn" onclick="stepUsedQty(' + idx + ',-1)" aria-label="' + (typeof t === 'function' ? t('wo.decrease') : 'Decrease') + '">−</button>' +
                '<input type="number" class="qty-input" min="1" max="' + stock + '" value="' + q + '" onchange="setUsedQty(' + idx + ', this.value)" aria-label="' + (typeof t === 'function' ? t('wo.qty_used') : 'Quantity used') + '">' +
                '<button type="button" class="qty-btn" onclick="stepUsedQty(' + idx + ',1)" aria-label="' + (typeof t === 'function' ? t('wo.increase') : 'Increase') + '">+</button>' +
            '</div>' +
            '<button type="button" class="wcc-part-remove" onclick="removeUsedPart(' + idx + ')" title="' + (typeof t === 'function' ? t('common.delete') : 'Remove') + '" aria-label="' + (typeof t === 'function' ? t('wo.remove_part') : 'Remove part') + '">✕</button>';
...
function validateTakeoverForm(form) {
    ...
    if (status && status.value === 'Completed') {
        ...
        for (let i = 0; i < chks.length; i++) {
            if (!chks[i].checked) {
                showToast(typeof t === 'function' ? t('wo.complete_checklist') : "Complete all checklist items by pressing and holding them before marking the work order as Completed.", "warning", 6000);
                return false;
            }
        }
    }
...
function filterUsedParts(query) {
    ...
    matches.forEach(part => {
        ...
        div.onclick = () => {
            ...
            if (isUnderMin) {
                openWccConfirm(typeof t === 'function' ? t('wo.under_min') : "Item under minimum threshold! Place a PR/PO! Proceed anyway?", addPart, typeof t === 'function' ? t('common.proceed') : 'Proceed');
                dropdown.style.display = 'none';
                document.getElementById('part_search').value = '';
                return;
            }
