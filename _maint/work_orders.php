...
try {
    ...
    switch ($sort) {
        ...
        case 'default':
        default:
            // Overdue first, using the shared predicate so the ordering, the row
            // highlight and the dashboard badge all agree on what "overdue" is.
            $order_by = "
                CASE WHEN " . wcc_wo_overdue_sql('w') . " THEN 1
    ...
    <script>
        ...
        function getColumnName(index) {
            try {
                ...
                if (!thead || !thead.children || index < 0 || index >= thead.children.length) return (typeof t === 'function' ? t('common.column') : 'Column');
                let th = thead.children[index];
                return th ? (th.innerText || th.textContent || '').trim() : (typeof t === 'function' ? t('common.column') : 'Column');
            } catch(e) { return (typeof t === 'function' ? t('common.column') : 'Column'); }
        }
        ...
        function resetSearchPosition() {
            try {
                ...
                input.style.width = '';
                input.placeholder = (typeof t === 'function' ? t('search.placeholder_wo') : 'Search work orders... (Drag to column)');
...
<?php include __DIR__ . '/../nav.php'; ?>
    <div class="dashboard-container">
        <div class="page-header" style="margin-bottom:10px;">
            ...
            <div id="searchWrapper">
                <div id="searchContainerOrig" style="display:inline-block; position:relative; width:100%;" draggable="true" ondragstart="dragSearch(event)">
                    ...
                    <input type="text" id="ledgerSearch" aria-label="<?= __e('search.aria_wo') ?>" onkeyup="handleSearchInput(event)" ondblclick="resetSearchPosition()" placeholder="<?= __e('search.placeholder_wo') ?>" style="padding:8px 35px 8px 35px; border-radius:20px; border: 1px solid var(--text-accent); background:var(--input-bg); color:var(--text-primary); transition: all 0.3s; box-sizing: border-box;">
                    <span id="lockTokenBtn" onclick="lockToken()" style="display:none; position:absolute; right:10px; top:50%; transform:translateY(-50%); cursor:pointer; font-size:1.1em; transition: transform 0.2s;" title="<?= __e('search.lock_token') ?>" onmouseover="this.style.transform='translateY(-50%) scale(1.2)'" onmouseout="this.style.transform='translateY(-50%) scale(1)'">🔒</span>
                </div>
            </div>
        </div>
        ...
        <div class="table-container" style="overflow-x:auto;">
            <table class="data-table" id="woTable">
                <thead>
                    <tr>
                        ...
                        <th ondrop="dropSearch(event, this)" ondragover="allowDrop(event)"><?= __e('th.equipment') ?></th>
                        <th ondrop="dropSearch(event, this)" ondragover="allowDrop(event)"><?= __e('th.assigned_to_cap') ?></th>
                        <th ondrop="dropSearch(event, this)" ondragover="allowDrop(event)"><?= __e('th.status') ?></th>
                        <th ondrop="dropSearch(event, this)" ondragover="allowDrop(event)">
            ...
            <?php 
                // Shared rule (inc/workorders.php) — identical to the dashboard badge.
                $is_overdue = wcc_wo_is_overdue($i, $db_today);
                $rowStyle = $is_overdue ? "background: var(--danger-bg); border-left: 3px solid var(--red-500);" : "";
            ?>
            <tr class="parent-row" data-id="<?= htmlspecialchars($i['wo_id']) ?>" style="<?= $rowStyle ?>">
                ...
                <td><?= htmlspecialchars($i['equip_name'] ?? __('common.unknown')) ?></td>
                <td>👨‍🔧 <?= htmlspecialchars($i['username'] ?? __('common.unassigned')) ?></td>
                <td>
                    <?php
                        $stat = $i['status'];
                        if ($is_overdue) {
                            echo "<span class='status-escalated'>" . __e('common.overdue') . "</span>";
                        } elseif ($stat === 'Completed') {
                            // Stored DB enum — display raw value
                            echo "<span class='status-closed'>" . htmlspecialchars($stat) . "</span>";
                        } elseif ($stat === 'Cancelled' || $stat === 'Missed') {
                            echo "<span class='status-open'>" . htmlspecialchars($stat) . "</span>";
                        } else {
                ...
                <td style="font-weight:bold;">
                    <?php if ($is_overdue): ?>
                        <span style="color: var(--danger); font-size: 0.9em; margin-right: 5px;">⚠️</span>
                    <?php endif; ?>
                    <?= htmlspecialchars($i['scheduled_date'] ?? __('common.tbd')) ?>
                </td>
            </tr>
            <tr class="child-row">
                <td colspan="6">
                    <div class="child-content">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
                            <div style="padding-right: 20px;">
                                <span style="font-weight: 800; color: var(--text-accent); font-size: 0.9em; text-transform: uppercase;"><?= __e('wo.instructions') ?></span><br>
                                <span style="font-size: 0.95em; line-height: 1.4; display: inline-block; margin-top: 5px;"><?= nl2br(htmlspecialchars($i['description'] ?? __('wo.no_instructions'))) ?></span>
                            </div>
                            <?php if ($i['status'] === 'Scheduled' || $i['status'] === 'In Progress' || $i['status'] === 'Overdue'): ?>
                                <a href="/_maint/wo_takeover.php?wo_id=<?= urlencode($i['wo_id']) ?>" class="btn btn-primary btn-sm">🚀 <?= __e('wo.takeover_btn') ?></a>
                            <?php endif; ?>
                        </div>
                        <?php 
                        // Accepts both stored shapes (see inc/partslist.php). Decoding
                        // raw here is what broke this page: a completed work order
                        // stores objects, and using one as an array key was fatal.
                        $parts = wcc_parts_list_decode($i['parts_list'] ?? null);
                        if (!empty($parts)):
                        ?>
                            <div style="border-top: 1px solid rgba(255,255,255,0.1); padding-top: 15px; margin-top: 5px;">
                                <span style="font-weight: 800; color: var(--text-accent); font-size: 0.9em; text-transform: uppercase; margin-bottom: 10px; display: block;"><?= __e('wo.required_parts') ?></span>
                                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                                    <?php foreach($parts as $part): ?>
                                        <div style="background: rgba(0,0,0,0.2); border: 1px solid var(--panel-border); padding: 5px 10px; border-radius: 6px; font-size: 0.85em; color: var(--text-secondary);">
                                            📦 <?= htmlspecialchars($parts_map[$part['part_id']] ?? __('wo.unknown_part')) ?><?php if ($part['qty'] > 1): ?>
                                                <strong style="color:var(--text-primary);">&times;<?= (int)$part['qty'] ?></strong>
                                            <?php endif; ?>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                        ...
                        ?>
                            <div style="border-top: 1px solid rgba(255,255,255,0.1); padding-top: 15px; margin-top: 15px;">
                                <span style="font-weight: 800; color: var(--text-accent); font-size: 0.9em; text-transform: uppercase; margin-bottom: 10px; display: block;"><?= __e('wo.checklist_audit') ?></span>
                                <div style="display: flex; flex-direction: column; gap: 8px;">
                                    <?php foreach($chk_data as $idx => $chk): ?>
                                        <div style="background: rgba(0,0,0,0.2); border: 1px solid var(--panel-border); padding: 8px 12px; border-radius: 6px; font-size: 0.9em; color: var(--text-primary); display: flex; justify-content: space-between; align-items: center;">
                                            <span>✔️ <?= htmlspecialchars($chk['task_desc'] ?? '') ?> <small style="color: var(--text-muted);">(<?= (int)($chk['expected_time_mins'] ?? 0) ?> <?= __e('common.mins') ?>)</small></span>
                                            <?php 
                                            $paths = [];
                                            ...
                                            ?>
                                                ...
                                                <?php foreach($paths as $pNum => $path): ?>
                                                    <button type="button" onclick="openImageOverlay('<?= htmlspecialchars($path) ?>')" style="background: var(--info-bg); border: 1px solid var(--info-border); color: var(--info); padding: 4px 10px; border-radius: var(--radius-sm); cursor: pointer; display: flex; align-items: center; gap: 5px; font-family: inherit;">📸 <?= count($paths) > 1 ? __e('wo.view_photo_n', ['n' => $pNum + 1]) : __e('wo.view_photo') ?></button>
                                                <?php endforeach; ?>
                                                </div>
                                            <?php endif; ?>
                                        </div>
...
<div id="imageOverlay" onclick="closeImageOverlay(event)" style="display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.8); z-index: 9999; justify-content: center; align-items: center; backdrop-filter: blur(5px); cursor: pointer;">
    <div style="position: relative; max-width: 90%; max-height: 90%; cursor: default;" onclick="event.stopPropagation()">
        <button type="button" onclick="document.getElementById('imageOverlay').style.display='none'" aria-label="<?= __e('common.close_photo') ?>" style="position: absolute; top: -15px; right: -15px; background: var(--red-500); color: white; width: 30px; height: 30px; border: none; border-radius: 50%; display: flex; justify-content: center; align-items: center; cursor: pointer; font-weight: bold; font-size: 18px; box-shadow: 0 4px 10px rgba(0,0,0,0.5);">×</button>
        <img id="overlayImg" src="" alt="<?= __e('wo.photo_alt') ?>" style="max-width: 100%; max-height: 90vh; border-radius: var(--radius-sm); box-shadow: 0 10px 30px rgba(0,0,0,0.8);">
    </div>
</div>
<script>
