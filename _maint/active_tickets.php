...
try {
    ...
    // Active fleet only (soft-deleted equipment excluded after migration 0005)
    try {
        $stmtEq = $pdo->query("SELECT COUNT(*) FROM equipment WHERE deleted_at IS NULL");
    } catch (Throwable $e) {
...
$page_title = __('ticket.active_title');
    ...
    async function submitComment(ticketId) {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                showToast(data.message || (typeof t === 'function' ? t('ticket.comment_failed') : 'Failed to add comment'), 'error');
            }
        } catch (err) {
            console.error(err);
            showToast(typeof t === 'function' ? t('ticket.comment_error') : 'Error submitting comment', 'error');
        }
    }
...
<div class="dashboard-container">
    <div class="page-header">
        <h1><?= __e('ticket.active_title') ?></h1>
        <div style="display:flex; gap:10px;"><a href="../index.php" class="nav-btn">🏠 <?= __e('common.hub') ?></a><a href="../register.php" class="nav-btn primary">+ <?= __e('ticket.new_ticket') ?></a></div>
    </div>
    ...
    <div class="health-panel">
        <div style="display: flex; justify-content: space-between; margin-bottom: 10px; align-items: flex-end; gap: var(--space-3); flex-wrap: wrap;">
            <span style="font-weight: 800; color: var(--text-accent); font-size: 1.1em; letter-spacing: 1px;">🏭 <?= __e('health.factory_title') ?></span>
            <span style="font-weight: 800; color: <?= $health_var ?>; font-size: 1.4em;"><?= __e('health.available_now', ['pct' => $health_pct]) ?></span>
        </div>
        <div style="width: 100%; background: rgba(0,0,0,0.2); border-radius: 10px; height: 12px; overflow: hidden; box-shadow: inset 0 1px 3px rgba(0,0,0,0.5);" role="img" aria-label="<?= __e('health.uptime_aria', ['pct' => $health_pct]) ?>">
    ...
    <table class="data-table table-cards" style="width: 100%;">
        <thead>
            <tr>
                <th><?= __e('th.ticket_id') ?></th>
                <th><?= __e('th.equipment_details') ?></th>
                <th><?= __e('th.priority') ?></th>
                <th><?= __e('th.status') ?></th>
                <th><?= __e('th.ongoing_time') ?></th>
                <th><?= __e('th.announced_by') ?></th>
                <th><?= __e('th.invoked_pic') ?></th>
                <th><?= __e('th.actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php if (count($tickets) > 0): ?>
                    ...
                    <tr class="<?= $rowClass ?> parent-row" data-id="<?= htmlspecialchars($ticket['ticket_id']) ?>">
                        ...
                        <td data-label="Equipment">
                            <div style="font-weight: 700; color: var(--text-accent); font-size: 1.1em;">
                                ...
                                <?php if($isRepeat): ?>
                                    <span class="repeat-chip">⚠️ <?= __e('ticket.repeat_offender') ?></span>
                                <?php endif; ?>
                            </div>
                            <div style="font-size: var(--fs-sm); color: var(--text-secondary); margin-top: 2px; font-weight: 500;"><?= htmlspecialchars($ticket['equip_name'] ?? __('ticket.unknown_machine')) ?></div>
                        </td>

                        <td data-label="<?= __e('th.priority') ?>"><span class="prio-badge <?= $badgeClass ?>"><?= $dot ?> <?= htmlspecialchars($prio) ?></span></td>

                        <td data-label="<?= __e('th.status') ?>">
                            <?php
                            $class = ($stat=='OPEN') ? 'status-open' : (($stat=='ESCALATED') ? 'status-escalated' : (($stat=='HOLD') ? 'status-escalated' : 'status-pending'));
                            if ($stat == 'HOLD') {
                                echo "<span class='$class hold-chip'>" . htmlspecialchars(__('ticket.on_hold')) . "</span>";
                            } else {
                                echo "<span class='$class'>" . htmlspecialchars($stat) . "</span>";
                            }

                            // The IDLE Warning Badge
                            if ($isIdle) {
                                echo "<br><span class='idle-chip'>⏳ " . htmlspecialchars(__('ticket.idle_badge')) . "</span>";
                            }
                            ?>
                        </td>

                        <td data-label="<?= __e('th.ongoing_time') ?>">
                            <div style="color: var(--text-secondary); font-size: var(--fs-sm);"><?= __e('ticket.form_date', ['date' => $ticket['report_time']]) ?></div>
                            <span class="live-timer" data-start="<?= $safe_timestamp ?>"><?= __e('common.calculating') ?></span>
                        </td>
                        <td data-label="<?= __e('th.announced_by') ?>"><?= htmlspecialchars($ticket['announced_by']) ?></td>

                        <td data-label="<?= __e('ticket.pic') ?>">
                            <div style="font-weight: 700; color: var(--text-accent);">👨‍🔧 <?= htmlspecialchars($ticket['pic'] ?? __('common.unassigned')) ?></div>
                        </td>

                        <td data-label="<?= __e('th.actions') ?>">
                            <?php if ($stat == 'OPEN' || $stat == 'ESCALATED'): ?>
                                <a href="/_maint/takeover.php?id=<?= urlencode($ticket['ticket_id']) ?>" class="action-btn btn-take"><?= __e('ticket.takeover_btn') ?></a>
                            <?php elseif ($stat == 'PENDING' || $stat == 'IN_PROGRESS'): ?>
                                <a href="/_maint/closeout.php?id=<?= urlencode($ticket['ticket_id']) ?>" class="action-btn btn-close"><?= __e('ticket.review_close') ?></a>
                                <button onclick="openHoldModal('<?= htmlspecialchars(addslashes($ticket['ticket_id'])) ?>'); event.stopPropagation();" class="action-btn" style="background: var(--warning-bg); color: var(--warning); border: 1px solid var(--warning-border); margin-top:5px; display:block; width:100%;"><?= __e('ticket.put_on_hold_btn') ?></button>
                            <?php elseif ($stat == 'HOLD'): ?>
                                <a href="/_maint/takeover.php?id=<?= urlencode($ticket['ticket_id']) ?>" class="action-btn btn-take"><?= __e('ticket.resume_job') ?></a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    ...
                    <tr class="child-row">
                        <td colspan="12">
                            <div class="child-content">
                                <span style="font-weight: 800; color: var(--text-accent); font-size: var(--fs-sm); text-transform: uppercase;"><?= __e('ticket.original_fault') ?></span><br>
                                ...
                                <?php if (!empty($actions_by_ticket[$ticket['ticket_id']])): ?>
                                    <div style="border-top: 1px solid var(--panel-border); padding-top: 15px; margin-top: 5px;">
                                        <span style="font-weight: 800; color: var(--text-accent); font-size: var(--fs-sm); text-transform: uppercase; margin-bottom: 10px; display: block;"><?= __e('ticket.intervention_timeline') ?></span>
                                ...
                                <div style="border-top: 1px solid var(--panel-border); padding-top: 15px; margin-top: 15px;">
                                    <span style="font-weight: 800; color: var(--text-accent); font-size: var(--fs-sm); text-transform: uppercase; margin-bottom: 10px; display: block;">💬 <?= __e('ticket.live_comments') ?></span>
                                    ...
                                    <div class="comments-container" id="commentsList_<?= htmlspecialchars($ticket['ticket_id']) ?>" style="max-height: 300px; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; margin-bottom: 10px; padding-right: 5px;">
                                        <?php if (!empty($comments_by_ticket[$ticket['ticket_id']])): ?>
                                            ...
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <div style="font-size: var(--fs-sm); color: var(--text-muted); font-style: italic;"><?= __e('ticket.no_comments_yet') ?></div>
                                        <?php endif; ?>
                                    </div>

                                    <!-- Add Comment Form -->
                                    <div style="display: flex; gap: 10px; margin-top: 10px;">
                                        <input type="text" id="commentInput_<?= htmlspecialchars($ticket['ticket_id']) ?>" placeholder="<?= __e('ticket.comment_placeholder') ?>" aria-label="<?= __e('ticket.comment_placeholder') ?> <?= htmlspecialchars($ticket['ticket_id']) ?>" style="flex-grow: 1; padding: 10px; border-radius: var(--radius-sm); border: 1px solid var(--text-accent); background: var(--input-bg); color: var(--text-primary); box-sizing: border-box;" onkeypress="if(event.key === 'Enter') submitComment('<?= htmlspecialchars(addslashes($ticket['ticket_id'])) ?>')">
                                        <button onclick="submitComment('<?= htmlspecialchars(addslashes($ticket['ticket_id'])) ?>')" class="action-btn btn-take" style="margin: 0; padding: 0 20px;"><?= __e('common.send') ?></button>
                                    </div>
                                </div>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="12"><div class="wcc-empty"><span class="empty-icon" aria-hidden="true">🎉</span><p><?= __e('ticket.no_active') ?></p></div></td></tr>
            <?php endif; ?>
        </tbody>
    </table>
    </div>
    ...
    <div class="page-header" style="margin-top: 50px; margin-bottom: 15px;">
        <h2 style="color: var(--text-accent); margin: 0; font-size: var(--fs-xl);">🛠️ <?= __e('ticket.pending_wos') ?></h2>
        <div>
            <a href="/_maint/pm_calendar.php" class="nav-btn primary">🗓️ <?= __e('ticket.open_pm_calendar') ?></a>
        </div>
    </div>
    ...
    <table class="data-table table-cards">
        <thead>
            <tr>
                <th><?= __e('th.wo_num') ?></th>
                <th><?= __e('th.title_instructions') ?></th>
                <th><?= __e('th.target_equipment') ?></th>
                <th><?= __e('th.scheduled_date') ?></th>
                <th><?= __e('th.assigned_tech') ?></th>
                <th><?= __e('th.actions') ?></th>
            </tr>
        </thead>
        <tbody>
                ...
                ?>
                    <tr style="<?= $rowStyle ?>">
                        <td data-label="WO #" style="font-weight: 600; color: var(--text-accent);">
                            ...
                            <?php if ($diff > 0): ?>
                                <br><span style="display:inline-block; margin-top:6px; font-size:var(--fs-xs); background: var(--danger-bg); color: var(--danger); padding: 3px 10px; border-radius: var(--radius-full); border: 1px solid var(--danger-border); font-weight: 800; letter-spacing: 0.5px;"><?= __e('common.overdue') ?></span>
                            <?php elseif ($diff == 0): ?>
                                <br><span style="display:inline-block; margin-top:6px; font-size:var(--fs-xs); background: var(--warning-bg); color: var(--warning); padding: 3px 10px; border-radius: var(--radius-full); border: 1px solid var(--warning-border); font-weight: 800; letter-spacing: 0.5px;"><?= __e('common.today') ?></span>
                            <?php endif; ?>
                        </td>
                        <td data-label="<?= __e('th.title') ?>"><strong style="color: var(--text-primary);"><?= htmlspecialchars($wo['title']) ?></strong>
                            <?php
                                $wo_stat = $wo['status'];
                        ...
                        <td data-label="<?= __e('nav.equipment') ?>"><?= htmlspecialchars($wo['equip_name'] ?? __('common.unknown')) ?></td>
                        <td data-label="<?= __e('th.scheduled_date') ?>" style="font-weight: bold;">
                            <?php if ($diff > 0): ?>
                        ...
                        <td data-label="<?= __e('th.assigned_to') ?>">👨‍🔧 <?= htmlspecialchars($wo['assigned_user'] ?? __('common.unassigned')) ?></td>
                        <td data-label="<?= __e('th.actions') ?>">
                            <a href="/_maint/wo_takeover.php?wo_id=<?= urlencode($wo['wo_id']) ?>" class="action-btn btn-take"><?= __e('ticket.takeover_btn') ?></a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="6"><div class="wcc-empty"><span class="empty-icon" aria-hidden="true">📭</span><p><?= __e('wo.no_pending') ?></p></div></td></tr>
            <?php endif; ?>
        </tbody>
    </table>
    </div>
...
<div id="holdModal" class="wcc-modal" role="dialog" aria-modal="true" aria-labelledby="holdModalTitle">
    <div class="wcc-modal-content wcc-modal-sm">
        <div class="wcc-modal-header">
            <h3 id="holdModalTitle"><?= __e('ticket.hold_modal_title') ?></h3>
            <button type="button" class="wcc-close-btn" onclick="closeWccModal('holdModal')" aria-label="<?= __e('btn.close') ?>">&times;</button>
        </div>
        <input type="hidden" id="hold_ticket_id">
        <div class="field">
            <label for="hold_reason"><?= __e('ticket.hold_reason') ?>:</label>
            <select id="hold_reason">
                <option value="Waiting for Parts"><?= __e('ticket.hold.waiting_parts') ?></option>
                <option value="Waiting for External Vendor"><?= __e('ticket.hold.waiting_vendor') ?></option>
                <option value="Waiting for Production Clearance"><?= __e('ticket.hold.waiting_production') ?></option>
                <option value="End of Shift / Handover"><?= __e('ticket.hold.end_of_shift') ?></option>
                <option value="Other"><?= __e('ticket.hold.other') ?></option>
            </select>
        </div>
        <div class="field">
            <label for="hold_explanation"><?= __e('ticket.hold_explanation') ?></label>
            <textarea id="hold_explanation" rows="3" placeholder="<?= __e('ticket.hold_details') ?>"></textarea>
        </div>
        <div class="wcc-modal-footer">
            <button type="button" class="btn" onclick="closeWccModal('holdModal')"><?= __e('btn.cancel') ?></button>
            <button type="button" class="btn btn-primary" onclick="submitHold()"><?= __e('ticket.hold_confirm') ?></button>
        </div>
    </div>
</div>
...
<script>
    ...
    async function submitHold() {
        ...
        if(!explanation) { showToast(typeof t === 'function' ? t('ticket.hold_explain') : 'Please provide an explanation.', 'warning'); return; }
        ...
        try {
            ...
            if (data.status === 'success') {
                if (typeof showToast === 'function') {
                    showToast(data.message || (typeof t === 'function' ? t('ticket.put_on_hold') : 'Ticket put on hold.'), 'success');
                }
                setTimeout(function () { window.location.reload(); }, 600);
            } else {
                showToast((typeof t === 'function' ? t('common.error') : 'Error') + ': ' + (data.message || (typeof t === 'function' ? t('common.action_failed') : 'Could not put ticket on hold.')), 'error');
            }
        } catch (e) {
            showToast(typeof t === 'function' ? t('common.network_error') : 'Network error.', 'error');
        }
    }
    ...
    function updateTimers() {
        ...
        timers.forEach(timer => {
            ...
            if (isNaN(startTime)) { timer.innerText = (typeof t === 'function' ? t('common.time_error') : 'Time Error'); return; }
            const diff = now - startTime;
            if (diff < 0) { timer.innerText = (typeof t === 'function' ? t('common.just_now') : 'Just now…'); return; }
