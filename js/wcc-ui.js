...
/* ==========================================================================
   0d. Search match count (drag-to-column / ledger search)
   --------------------------------------------------------------------------
   Pages with #ledgerSearch + #activeFiltersArea should include:
     <span id="searchMatchCount" class="search-match-count" aria-live="polite"></span>
   as the last child of #activeFiltersArea. filterTable() updates it after
   showing/hiding parent rows. Token chips must insert before the count.
   ========================================================================== */

/** Keep match-count rightmost when appending a filter-token chip. */
function wccAppendFilterToken(area, tokenEl) {
    if (!area || !tokenEl) return;
    var count = document.getElementById('searchMatchCount');
    if (count && count.parentNode === area) {
        area.insertBefore(tokenEl, count);
    } else {
        area.appendChild(tokenEl);
    }
}
window.wccAppendFilterToken = wccAppendFilterToken;

/**
 * Set #searchMatchCount label.
 * @param {number} visible
 * @param {number} total
 * @param {boolean} [filtering]
 */
function wccUpdateSearchMatchCount(visible, total, filtering) {
    var el = document.getElementById('searchMatchCount');
    if (!el) return;
    total = total | 0;
    visible = visible | 0;
    ...
    if (filtering) {
        el.textContent = (typeof t === 'function')
            ? t('search.n_of_total', { visible: visible, total: total })
            : (visible + ' of ' + total);
        el.setAttribute('data-filtering', '1');
    } else {
        el.textContent = (typeof t === 'function')
            ? t('search.n_records', { total: total })
            : (total + ' records');
        el.setAttribute('data-filtering', '0');
    }
}
window.wccUpdateSearchMatchCount = wccUpdateSearchMatchCount;

/**
 * Count rows whose style.display is not 'none' and update the match label.
 * @param {HTMLCollection|NodeList|Array} rows
 * @param {boolean} filtering
 * @param {{skipSingleCell?: boolean}} [opts] skip empty-state colspan rows
 */
function wccRefreshSearchMatchCount(rows, filtering, opts) {
    opts = opts || {};
    if (!rows) {
        wccUpdateSearchMatchCount(0, 0, !!filtering);
        return;
    }
    var total = 0;
    var visible = 0;
    ...
    for (var i = 0; i < n; i++) {
        var r = rows[i];
        if (!r) continue;
        if (opts.skipSingleCell && r.children && r.children.length === 1) continue;
        if (r.closest && r.closest('thead')) continue;
        total++;
        if (!r.style || r.style.display !== 'none') visible++;
    }
    wccUpdateSearchMatchCount(visible, total, filtering);
}
window.wccRefreshSearchMatchCount = wccRefreshSearchMatchCount;
...
function wccUpdateThemeToggleUI() {
    ...
    if (label) {
        if (typeof t === 'function') {
            label.textContent = isLight ? t('nav.theme_light') : t('nav.theme_dark');
        } else {
            label.textContent = isLight ? 'Theme: Light' : 'Theme: Dark';
        }
    }
    btn.setAttribute('aria-pressed', isLight ? 'true' : 'false');
    var switchTitle = (typeof t === 'function') ? t('nav.switch_theme') : (isLight ? 'Switch to dark theme' : 'Switch to light theme');
...
function closeAboutModal() {
    ...
    if (typeof closeWccSupportModal === 'function') closeWccSupportModal();
}

/* Esc closes any open wcc-modal; backdrop click closes too */
document.addEventListener('keydown', function (e) {
    if (e.key !== 'Escape') return;
    var support = document.getElementById('wccSupportModal');
    if (support && support.classList.contains('open')) {
        if (typeof closeWccSupportModal === 'function') closeWccSupportModal();
        return;
    }
...
/* ==========================================================================
   ...
   ========================================================================== */
/**
 * Lightweight confetti burst (no external deps). Used for happy moments
 * like “coffee + hide for 30 days”. Safe to call anytime.
 */
function wccConfettiBurst(opts) {
    opts = opts || {};
    var count = opts.count || 80;
    var duration = opts.duration || 2200;
    var colors = opts.colors || ['#a78bfa', '#34d399', '#fbbf24', '#60a5fa', '#f472b6', '#f97316', '#ffffff'];
    var root = document.createElement('div');
    root.setAttribute('aria-hidden', 'true');
    root.style.cssText = 'position:fixed;inset:0;overflow:hidden;pointer-events:none;z-index:110020;';
    ...
    for (var i = 0; i < count; i++) {
        ...
        p.style.cssText = [
            ...
            'animation:wccConfettiFall ' + (duration / 1000) + 's cubic-bezier(0.22,0.7,0.35,1) ' + delay + 's forwards'
        ].join(';');
        p.style.setProperty('--wcc-cx', drift + 'px');
        root.appendChild(p);
        pieces.push(p);
    }
    if (!document.getElementById('wccConfettiStyle')) {
        var st = document.createElement('style');
        st.id = 'wccConfettiStyle';
        st.textContent = '@keyframes wccConfettiFall{0%{transform:translate3d(0,-10px,0) rotate(0deg);opacity:1}100%{transform:translate3d(var(--wcc-cx,0),110vh,0) rotate(720deg);opacity:0}}';
 ...
 * @param {string} [iconOverride] optional emoji/icon (e.g. 💨) instead of type default
 */
function showToast(message, type, timeoutMs, iconOverride) {
    type = type || 'info';
    ...
    icon.textContent = (iconOverride != null && String(iconOverride) !== '')
        ? String(iconOverride)
