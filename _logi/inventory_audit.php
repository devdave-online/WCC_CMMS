<?php
// Same gate as inventory list / nav: only RBAC holders of view_inventory may
// audit stock movements (direct URL must not bypass the sidebar).
...
try {
    ...
    error_log('[WCC inventory_audit] ' . $e->getMessage());
    die('Database error. Check server logs.');
}
?>
<?php
$page_title = __('inv.audit_title');
    ...
    <script>
        ...
        function filterTable() {
            ...
            if (typeof wccRefreshSearchMatchCount === 'function') {
                wccRefreshSearchMatchCount(tr, globalFilter !== '' || activeFilters.length > 0, { skipSingleCell: true });
            }
        }
        ...
        function resetSearchPosition() {
            ...
            input.style.width = '';
            input.placeholder = (typeof t === 'function' ? t('search.placeholder_audit') : 'Search transactions... (Drag to column)');
        ...
        <div style="display:flex; gap:10px; align-items:center;">
            <div id="searchWrapper">
                <div id="searchContainerOrig" style="display:inline-block; position:relative; width:100%;" draggable="true" ondragstart="dragSearch(event)">
                    ...
                    <input type="text" id="ledgerSearch" onkeyup="handleSearchInput(event)" ondblclick="resetSearchPosition()" placeholder="<?= __e('search.placeholder_audit') ?>" style="padding:8px 35px 8px 35px; border-radius:20px; border: 1px solid var(--text-accent); background:var(--input-bg); color:var(--text-primary); transition: all 0.3s; box-sizing: border-box;">
            ...
            <tbody>
                <?php foreach ($transactions as $t): 
                    ...
                    $reasonMap = [
                        ...
                        'ticket_consume' => 'Ticket Consumption',
