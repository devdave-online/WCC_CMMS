 ...
 *   wcc_notify_perms(['view_history','view_statistics'], 'ticket_closed', '…', '/_rpt/history.php', 'success', $exclude_uid);
 */
...
{
    return wcc_notify_perms([$permission], $type, $message, $link, $severity, $exclude_uid);
}

/**
 * Notify every active user who holds ANY of the given permissions — one row per user
 * (union / dedupe). Use when an event is relevant to multiple capabilities (e.g.
 * view_history + view_statistics) without double-bell spam.
 *
 * @param string[] $permissions
 */
function wcc_notify_perms(array $permissions, string $type, string $message, ?string $link = null, string $severity = 'info', ?int $exclude_uid = null): int
{
    $sent = 0;
    $permissions = array_values(array_filter(array_map('strval', $permissions), static function ($p) {
        return $p !== '';
    }));
    if ($permissions === []) {
        return 0;
    ...
    try {
        ...
        foreach ($rows as $u) {
            $uid = (int)$u['user_id'];
            if ($exclude_uid !== null && $uid === $exclude_uid) {
                continue;
            }
        ...
        error_log('[WCC NOTIFY_PERMS] ' . $type . ': ' . $e->getMessage());
    }
    return $sent;
}
