...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        elseif ($action === 'modify_budget') {
            ...
            // Never allow a negative amount, and never let a Revoke/Relocate drive
            // a department's allocation below zero (which would corrupt budgets and,
            // for Relocate, mint phantom money into the target).
            $current_allocated = (float)$pdo->query("SELECT budget_allocated FROM departments WHERE dept_id = " . (int)$dept_id)->fetchColumn();
            if ($amount <= 0) {
                header("Location: /_mgmt/setup_vault_departments.php?err=amount");
                exit;
            }
            if (($mod_action === 'Revoke' || $mod_action === 'Relocate') && $amount > $current_allocated) {
                header("Location: /_mgmt/setup_vault_departments.php?err=insufficient");
                exit;
            }
...
?>
<?php
$page_title = __('dept.vault_title');
...
<?php include __DIR__ . '/../nav.php'; ?>
    ...
    <div class="dashboard-container">
        <div class="page-header">
            ...
            <div style="display:flex; gap:15px;">
                ...
                <?php endif; ?>
                <button class="pill-btn pill-success" style="white-space:nowrap;" onclick="document.getElementById('addModal').style.display='block'">+ Add Budget Vault</button>
            </div>
        </div>
        ...
        <div class="vault-grid">
            ...
            <div class="dept-card">
                <div class="dept-header">
                    ...
                    <a href="#" onclick="openWccConfirm('Delete this Budget Vault? (Historical Purchase Orders will be preserved but unlinked from this vault)', '?delete_id=<?= $d['dept_id'] ?>&csrf=<?= wcc_csrf_token() ?>'); return false;" style="color:#ef4444; font-size:0.9em; text-decoration:none;">✖ Delete</a>
                </div>
                <div class="stat-grid">
                    <div class="stat-box">
                ...
                <div class="actions-flex">
                    <button class="pill-btn pill-success" style="flex:1;" onclick="openTransactionModal(<?= $d['dept_id'] ?>, '<?= addslashes(htmlspecialchars($d['dept_name'])) ?>')">Modify Budget</button>
                    <button class="pill-btn pill-info" style="flex:1;" onclick="openHistoryModal(<?= $d['dept_id'] ?>, '<?= addslashes(htmlspecialchars($d['dept_name'])) ?>')">Transaction History</button>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
    ...
    <div id="addModal" class="modal">
        <div class="modal-content" style="max-width:500px;">
            ...
            <form method="POST">
                ...
                <button type="submit" class="pill-btn pill-success pill-block" style="margin-top:20px;">💾 Save Budget Vault</button>
            </form>
        </div>
    </div>
    ...
    <div id="modModal" class="modal">
        <div class="modal-content" style="max-width:500px;">
            ...
            <h2 id="modTitle"><?= __e('dept.transaction') ?></h2>
            <form method="POST">
                ...
                <button type="submit" class="pill-btn pill-success pill-block" style="margin-top:20px;">Execute Transaction</button>
            </form>
        </div>
    </div>
    ...
    <div id="histModal" class="modal">
        <div class="modal-content" style="max-width:800px;">
            ...
            <h2 id="histTitle"><?= __e('dept.history') ?></h2>
