...
?>
<?php
$page_title = __('settings.console_title');
...
<div class="dashboard-container">
    <div class="page-header">
        <h1>⚙️ <?= __e('settings.console_title') ?></h1>
        <div style="display:flex; align-items:center; gap:12px;">
            <?php if (can('manage_settings')): ?>
            <a href="/_mgmt/admin_panel.php" class="nav-btn" style="white-space:nowrap;">← <?= __e('settings.return_admin') ?></a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <?php if (isset($_GET['msg'])): ?>
        <?php
        $flash = [
            'lockout_updated'  => ['ok', __('settings.msg.lockout_updated')],
            'holidays_updated' => ['ok', __('settings.msg.holidays_updated')],
        ];
        $mk = (string)$_GET['msg'];
        if (isset($flash[$mk])):
            [$tone, $text] = $flash[$mk];
            $bg = $tone === 'ok' ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)';
            $fg = $tone === 'ok' ? '#10b981' : '#ef4444';
        ?>
        <div style="margin-bottom:16px; padding:12px 16px; border-radius:10px; background:<?= $bg ?>; color:<?= $fg ?>; border:1px solid <?= $fg ?>33;">
    ...
    <div style="margin-top: 40px; padding: 20px; background: var(--panel-bg); border-radius: 16px; border: 1px solid var(--panel-border); box-shadow: 0 10px 30px 0 rgba(0,0,0,0.2);">
        <h3 style="color: var(--text-accent); margin-top: 0;"><?= __e('settings.security') ?></h3>
        <form method="POST" style="display: flex; align-items: center; gap: 15px;">
            ...
            <label style="color:var(--text-secondary);"><?= __e('settings.session_lockout') ?></label>
            <input type="number" name="session_lockout_time" value="<?= htmlspecialchars((string)$lockout_time) ?>" required min="1" max="1440" style="padding:10px; border-radius:6px; background:rgba(0,0,0,0.1); border:1px solid var(--panel-border); color:var(--text-primary); font-size:1.1em; width: 100px;">
            <button type="submit" class="pill-btn pill-success">💾 <?= __e('settings.save_changes') ?></button>
        </form>
        <p style="color:var(--text-secondary); font-size:0.85em; margin-top: 10px;"><?= __e('settings.session_lockout_help') ?></p>

        <!-- Operational Calendar (Plant Holidays) -->
        <h3 style="color: var(--text-accent); margin-top: 40px; margin-bottom: 15px;">📅 <?= __e('settings.ops_calendar') ?></h3>
        <form method="POST" style="background: rgba(0,0,0,0.1); padding: 20px; border-radius: 12px; border: 1px solid var(--panel-border);">
            ...
            <div style="margin-bottom: 15px;">
                <label style="color:var(--text-secondary); display:block; margin-bottom: 5px;"><?= __e('settings.plant_holidays') ?></label>
                ...
                <p style="color:var(--text-secondary); font-size: 0.85em; margin-top: 5px;"><?= __e('settings.plant_holidays_help') ?></p>
            </div>
            <button type="submit" class="pill-btn pill-success">💾 <?= __e('settings.save_holidays') ?></button>
        </form>
            ...
            <table class="data-table" style="font-size: 0.9em;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="#" onclick="openWccConfirm('Deleting a workshop will delete ALL its lines too! Proceed?', '?delete_workshop=<?= $w['workshop_id'] ?>&csrf=<?= wcc_csrf_token() ?>', 'Delete Workshop'); return false;" class="pill-btn pill-danger pill-sm">✕</a></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
