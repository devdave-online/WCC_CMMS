...
try {
    ...
    if (!isset($data['ticket_id'])) { echo json_encode(['status' => 'error', 'message' => __('ticket.invalid_data')]); exit; }
    ...
    // Escalate: notify anyone with takeover or closeout (all roles that hold those perms)
    if ($new_status === 'ESCALATED') {
        require_once __DIR__ . '/../inc/notifications.php';
        $actorUid = (int)($_SESSION['user_id'] ?? 0);
        $ticketId = (string)$data['ticket_id'];
        $escTo = trim((string)($data['escalated_to'] ?? ''));
        $msg = 'Ticket escalated: ' . $ticketId
            . ' (by ' . $locked_tech_name . ')'
            . ($escTo !== '' && strcasecmp($escTo, 'None') !== 0 ? ' → ' . $escTo : '');
        wcc_notify_perms(
            ['takeover_tickets', 'closeout_tickets'],
            'ticket_escalated',
            $msg,
    ...
    echo json_encode(['status' => 'success', 'message' => __('ticket.action_logged')]);
} catch (PDOException $e) {
    echo json_encode(['status' => 'error', 'message' => __('ticket.could_not_takeover')]);
}
?>
