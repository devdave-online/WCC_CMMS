    ...
    switch ($method) {
        case 'GET':
            if ($id) {
                ...
                $asset_uuid = $_GET['asset_uuid'] ?? null;
                $search = $_GET['search'] ?? '';
                ...
                // Exact asset tag lookup (QR/barcode scan from the companion app).
                if ($asset_uuid) {
                    $sql .= " AND asset_uuid = ?";
                    $params[] = $asset_uuid;
                }
                // Free-text search across name / asset tag / model.
                if ($search) {
                    $sql .= " AND (equip_name LIKE ? OR asset_uuid LIKE ? OR oem_model LIKE ?)";
                    $params[] = "%$search%";
                    $params[] = "%$search%";
                    $params[] = "%$search%";
                }
