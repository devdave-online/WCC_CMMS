...
function handle_ai_context($method, $id, $input) {
    ...
    $response = [
        ...
        'instructions' => 'Read _ai_ctxt/AGENT_INSTRUCTIONS.md then PRODUCT_STATUS.md first. Bootstrap: ai_agent.ini. All context files are in _ai_ctxt/',
        'release' => 'OB1.0.0 open beta — offline one-site installs, no hardlocks',
        'sections' => []
    ];
...
function get_safe_live_context($pdo) {
    ...
    $tables = [
        ...
        'equipment' => 'SELECT COUNT(*) FROM equipment WHERE deleted_at IS NULL',
        'toolings' => 'SELECT COUNT(*) FROM toolings WHERE deleted_at IS NULL',
        'active_tickets' => 'SELECT COUNT(*) FROM active_tickets WHERE status IN ("OPEN","PENDING","ESCALATED","HOLD")',
    ...
    try {
        $stmt = $pdo->query("SELECT equip_id, equip_name, category, is_active FROM equipment WHERE deleted_at IS NULL ORDER BY equip_id DESC LIMIT 3");
        $live['sample_equipment'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $e) {}

    try {
        $stmt = $pdo->query("SELECT tooling_id, tooling_code, tooling_name, status, category FROM toolings WHERE deleted_at IS NULL ORDER BY tooling_id DESC LIMIT 3");
        $live['sample_toolings'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $e) {}
