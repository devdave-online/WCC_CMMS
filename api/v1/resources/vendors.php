...
function handle_vendors($method, $id, $input) {
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            // Column is vendor_address (not address)
            $stmt = $pdo->prepare("INSERT INTO vendors_suppliers (vendor_name, primary_contact_name, contact_email, contact_phone, vendor_address) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([
                $input['vendor_name'],
                $input['primary_contact_name'] ?? null,
                $input['contact_email'] ?? null,
                $input['contact_phone'] ?? null,
                $input['vendor_address'] ?? $input['address'] ?? null
            ]);
        ...
        case 'PATCH':
            ...
            $fields = [];
            $params = [];
            $allowed = ['vendor_name', 'primary_contact_name', 'contact_email', 'contact_phone', 'vendor_address', 'vendor_remarks', 'payment_terms', 'vendor_type', 'rating'];
            // Accept legacy "address" / "notes" aliases from clients
            if (isset($input['address']) && !isset($input['vendor_address'])) {
                $input['vendor_address'] = $input['address'];
            }
            if (isset($input['notes']) && !isset($input['vendor_remarks'])) {
                $input['vendor_remarks'] = $input['notes'];
            }
