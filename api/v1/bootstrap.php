...
// Shared identity + id allocation, so REST writes follow exactly the same rules as
// the web app (display-name stamping, collision-safe ticket ids, alias matching).
require_once __DIR__ . '/../../inc/techident.php';
require_once __DIR__ . '/../../inc/ticketid.php';
// NOTE: do NOT include auth.php here — it redirects browser-less requests to
// /login.php, which pre-empts this API's own X-API-Key / Basic auth
// (api_authenticate()). We only need a session + the RBAC engine.
...
/**
 * True if the user row may authenticate (status active, or legacy NULL/empty).
 */
function api_user_is_active(array $user): bool {
...
function api_authenticate() {
    ...
    if ($apiKey) {
        ...
        if ($user) {
            if (!api_user_is_active($user)) {
                api_error('Forbidden: account is inactive or pending.', 403);
            }
            $_SESSION['user_id'] = $user['user_id'];
            $_SESSION['username'] = $user['username'];
            $_SESSION['badge_number'] = $user['badge_number'] ?? null;
