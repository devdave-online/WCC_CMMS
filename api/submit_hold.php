...
try {
    ...
    // Hold: notify anyone who can take over or close out (all roles with those perms)
    require_once __DIR__ . '/../inc/notifications.php';
    $actorUid = (int)($_SESSION['user_id'] ?? 0);
    $msg = 'Ticket on hold: ' . $ticket_id . ' — ' . $reason
        . ' (by ' . $tech_name . ')';
    wcc_notify_perms(
    ...
    echo json_encode(['status' => 'success', 'message' => __('ticket.put_on_hold')]);
