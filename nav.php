...
$__notif_aria   = __e('nav.notifications') . ($__notif_unread > 0 ? ' ' . __e('notif.unread_suffix', ['n' => $__notif_unread]) : '');
...
<script>
window.WCC_LOCALE = window.WCC_LOCALE || <?= json_encode(wcc_locale(), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
window.WCC_I18N = window.WCC_I18N || <?= json_encode(wcc_i18n_js_dict(), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
window.WCC_I18N_FALLBACK = window.WCC_I18N_FALLBACK || <?= json_encode(wcc_i18n_js_fallback(), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;
</script>
...
<button type="button" id="mobileNavToggle" onclick="toggleSidebar()" aria-label="<?= __e('nav.open_menu') ?>">☰</button>

<nav class="wcc-sidebar" id="wccSidebar" aria-label="<?= __e('nav.main') ?>">
    <div class="anchored-pointer"></div>
    <div class="wcc-brand">
        <button type="button" class="menu-toggle" onclick="toggleSidebar()" aria-label="<?= __e('nav.toggle_sidebar') ?>">☰</button>
        <span class="brand-text" id="wccBrandText" style="cursor: pointer;" onclick="openAboutModal()" title="<?= __e('nav.about') ?>">
            <img src="/img/wcc-orb.png" alt="" class="wcc-brand-mark" width="28" height="28" decoding="async">
            <?= __e('app.short') ?>
        </span>
    </div>
    ...
    <div class="nav-section"><?= __e('nav.section.operations') ?></div>
    <?php endif; ?>
    ...
    <a href="/index.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'index.php' || basename($_SERVER['PHP_SELF']) == 'active_tickets.php' ? 'active' : '' ?>">
        <span class="icon">🎫</span><span class="text"><?= __e('nav.tickets') ?></span>
    </a>
    <?php endif; ?>
    ...
    <a href="/_maint/work_orders.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'work_orders.php' ? 'active' : '' ?>">
        <span class="icon">🛠️</span><span class="text"><?= __e('nav.work_orders') ?></span>
    </a>
    <a href="/_maint/pm_calendar.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'pm_calendar.php' ? 'active' : '' ?>">
        <span class="icon">🗓️</span><span class="text"><?= __e('nav.pm_calendar') ?></span>
    </a>
    <?php endif; ?>

    <!-- Assets -->
    <?php if(can('view_equipment') || can('view_toolings') || can('view_inventory')): ?>
    <div class="nav-section"><?= __e('nav.section.assets') ?></div>
    <?php endif; ?>
    ...
    <a href="/_eam/equipment_list.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'equipment_list.php' || basename($_SERVER['PHP_SELF']) == 'equipment.php' ? 'active' : '' ?>">
        <span class="icon">⚙️</span><span class="text"><?= __e('nav.equipment') ?></span>
    </a>
    <?php endif; ?>

    <?php if(can('view_toolings')): ?>
    <a href="/_eam/toolings.php" class="wcc-nav-link <?= in_array(basename($_SERVER['PHP_SELF']), ['toolings.php','toolings_list.php','setup_vault_toolings.php'], true) ? 'active' : '' ?>">
        <span class="icon">🔧</span><span class="text"><?= __e('nav.tooling') ?></span>
    </a>
    <?php endif; ?>
    ...
    <a href="/_prod/setup_vault_lines.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'setup_vault_lines.php' ? 'active' : '' ?>">
        <span class="icon">🏭</span><span class="text"><?= __e('nav.prod_lines') ?></span>
    </a>
    <?php endif; ?>

    <?php if(can('view_inventory')): ?>
    <a href="/_logi/inventory.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'inventory.php' ? 'active' : '' ?>">
        <span class="icon">📦</span><span class="text"><?= __e('nav.inventory') ?></span>
    </a>
    <?php endif; ?>
    ...
    <div class="nav-section"><?= __e('nav.section.records') ?></div>
    <?php endif; ?>

    <?php if(can('view_history')): ?>
    <a href="/_rpt/history.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'history.php' ? 'active' : '' ?>">
        <span class="icon">🗄️</span><span class="text"><?= __e('nav.event_history') ?></span>
    </a>
    <?php endif; ?>
    ...
    <a href="/_logi/vendors_list.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'vendors_list.php' || basename($_SERVER['PHP_SELF']) == 'vendors.php' ? 'active' : '' ?>">
        <span class="icon">🏢</span><span class="text"><?= __e('nav.vendors') ?></span>
    </a>
    <?php endif; ?>
    ...
    <a href="/_logi/purchase_requests.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'purchase_requests.php' ? 'active' : '' ?>">
        <span class="icon">🛒</span><span class="text"><?= __e('nav.purchase_requests') ?></span>
    </a>
    <?php endif; ?>
    ...
    <div class="nav-section"><?= __e('nav.section.people') ?></div>
    <a href="/_mgmt/users_list.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'users_list.php' || basename($_SERVER['PHP_SELF']) == 'users.php' ? 'active' : '' ?>">
        <span class="icon">👥</span><span class="text"><?= __e('nav.users') ?></span>
    </a>
    <?php endif; ?>

    <!-- Insights -->
    <?php if(can('view_statistics')): ?>
    <div class="nav-section"><?= __e('nav.section.insights') ?></div>
    <a href="/_rpt/statistics.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'statistics.php' ? 'active' : '' ?>">
        <span class="icon">📊</span><span class="text"><?= __e('nav.analytics') ?></span>
    </a>
    <?php endif; ?>
    ...
    <div class="wcc-sidebar-footer">
        ...
        <div class="sidebar-widget-container">
            <a href="/my_profile.php" class="collapsed-widget sidebar-profile-icon <?= basename($_SERVER['PHP_SELF']) == 'my_profile.php' ? 'active' : '' ?>" title="<?= __e('nav.my_profile') ?>" aria-label="<?= __e('nav.my_profile') ?>">
            ...
            <a href="/my_profile.php" class="expanded-widget sidebar-profile-card <?= basename($_SERVER['PHP_SELF']) == 'my_profile.php' ? 'active' : '' ?>">
                <div class="sidebar-profile-meta">
                    ...
                    <span class="meta-key"><?= __e('nav.role') ?></span> <span class="meta-role"><?= $role_name ?></span><br>
                    <span class="meta-key"><?= __e('nav.user') ?></span> <span class="meta-user"><?= isset($_SESSION['badge_number']) ? htmlspecialchars($_SESSION['badge_number']) : (isset($_SESSION['username']) ? htmlspecialchars(strtoupper($_SESSION['username'])) : 'GUEST') ?></span>
                </div>
            </a>
        </div>

        <?php if(can('manage_settings')): ?>
        <div class="nav-section"><?= __e('nav.section.admin') ?></div>
        <a href="/_mgmt/admin_panel.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'admin_panel.php' ? 'active' : '' ?>">
            <span class="icon">🛡️</span><span class="text"><?= __e('nav.admin_panel') ?></span>
        </a>
        <a href="/_mgmt/app_settings.php" class="wcc-nav-link <?= basename($_SERVER['PHP_SELF']) == 'app_settings.php' ? 'active' : '' ?>">
            <span class="icon">⚙️</span><span class="text"><?= __e('nav.settings') ?></span>
        </a>
        <?php endif; ?>

        <button type="button" id="wccNotifBell" class="wcc-nav-link" onclick="openWccModal('wccNotifModal')" title="<?= __e('nav.notifications') ?>" aria-label="<?= $__notif_aria ?>">
            <span class="icon wcc-notif-icon" aria-hidden="true">🔔<?php if($__notif_unread > 0): ?><span class="wcc-notif-dot"></span><?php endif; ?></span>
            <span class="text"><?= __e('nav.notifications') ?><?php if($__notif_unread > 0): ?> <span class="wcc-notif-count"><?= $__notif_unread ?></span><?php endif; ?></span>
        </button>

        <button type="button" id="wccThemeToggle" class="wcc-nav-link" onclick="toggleTheme()" aria-pressed="false" title="<?= __e('nav.switch_theme') ?>">
            <span class="icon" id="wccThemeIcon" aria-hidden="true">🌙</span><span class="text" id="wccThemeLabel"><?= __e('nav.theme_dark') ?></span>
        </button>

        <a href="/login.php?logout=true" class="wcc-nav-link wcc-nav-logout">
            <span class="icon">🚪</span><span class="text"><?= __e('nav.logout') ?></span>
        </a>
    </div>
</nav>
...
<div class="wcc-modal" id="wccNotifModal" role="dialog" aria-modal="true" aria-labelledby="wccNotifTitle">
    <div class="wcc-modal-content wcc-modal-md">
        <div class="wcc-modal-header">
            <h3 id="wccNotifTitle">🔔 <?= __e('notif.title') ?></h3>
            <button type="button" class="wcc-close-btn" onclick="closeWccModal('wccNotifModal')" aria-label="<?= __e('notif.close') ?>">✕</button>
        </div>
        <div class="wcc-notif-list" id="wccNotifList">
            <?php if(empty($__notif_list)): ?>
                <div class="wcc-notif-empty">🌱 <?= __e('notif.empty') ?></div>
        ...
        <div class="wcc-modal-footer">
            <button type="button" class="pill-btn" onclick="wccNotifAction('mark_all_read')"><?= __e('notif.mark_all_read') ?></button>
            <button type="button" class="pill-btn pill-danger" onclick="wccNotifAction('delete_all')"><?= __e('notif.delete_all') ?></button>
        </div>
    </div>
</div>
<script>
    ...
    async function wccNotifAction(action) {
        try {
            ...
            else { showToast(res.message || (typeof t === 'function' ? t('notif.action_failed') : 'Action failed.'), 'error'); }
        } catch (e) { showToast(typeof t === 'function' ? t('notif.update_failed') : 'Could not update notifications.', 'error'); }
    }
