...
?>
<?php
$page_title = __('equip.ledger_title');
...
?>
    ...
    <script>
        ...
        function resetSearchPosition() {
            ...
            input.style.width = '';
            input.placeholder = (typeof t === 'function' ? t('search.placeholder_equipment') : 'Search by name, uuid, brand... (Drag to column)');
            ...
            <div id="searchWrapper">
                <div id="searchContainerOrig" style="display:inline-block; position:relative; width:100%;" draggable="true" ondragstart="dragSearch(event)">
                    <span style="position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--text-secondary); pointer-events:none; font-size:1.1em;" aria-hidden="true">🔍</span>
                        <input type="text" id="ledgerSearch" aria-label="<?= __e('search.aria_equipment') ?>" style="padding:8px 35px 8px 35px; border-radius:20px; border: 1px solid var(--text-accent); background:var(--input-bg); color:var(--text-primary); transition: all 0.3s; box-sizing: border-box;" onkeyup="handleSearchInput(event)" placeholder="<?= __e('search.placeholder_equipment') ?>"
