...
$page_title = __('tooling.registry_title');
...
?>
    ...
    <script>
        ...
        function resetSearchPosition() {
            ...
            input.style.width = '';
            input.placeholder = (typeof t === 'function' ? t('search.placeholder_tooling') : 'Search by name, code, tag... (Drag to column)');
...
<?php include __DIR__ . '/../nav.php'; ?>
    <div class="dashboard-container">
        <div class="page-header">
            <h1><?= __e('tooling.registry_title') ?></h1>
            <div id="searchWrapper">
                <div id="searchContainerOrig" style="display:inline-block; position:relative; width:100%;" draggable="true" ondragstart="dragSearch(event)">
                    <span style="position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--text-secondary); pointer-events:none; font-size:1.1em;" aria-hidden="true">🔍</span>
                        <input type="text" id="ledgerSearch" aria-label="<?= __e('search.aria_tooling') ?>" style="padding:8px 35px 8px 35px; border-radius:20px; border: 1px solid var(--text-accent); background:var(--input-bg); color:var(--text-primary); transition: all 0.3s; box-sizing: border-box;" onkeyup="handleSearchInput(event)" placeholder="<?= __e('search.placeholder_tooling') ?>"
...
<div id="wccBomModal" class="wcc-modal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); z-index: 9999; align-items: center; justify-content: center;">
    <div class="wcc-modal-content" style="background: var(--panel-bg); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 30px; width: 90%; max-width: 600px; box-shadow: 0 15px 40px rgba(0,0,0,0.4);">
        ...
        <input type="text" id="bomSearch" placeholder="<?= __e('search.placeholder_linked_parts') ?>" style="width: 100%; padding: 10px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1); background: rgba(0,0,0,0.2); color: white; margin-bottom: 15px; box-sizing: border-box;" onkeyup="filterBOM()">
