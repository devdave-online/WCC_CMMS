...
<?php include __DIR__ . '/../nav.php'; ?>
    <div class="dashboard-container">
        <div class="page-header" style="margin-bottom:10px;">
            <h1>Work Orders (Scheduled Maintenance)</h1>
            <div id="searchWrapper">
                <div id="searchContainerOrig" style="display:inline-block; position:relative; width:100%;" draggable="true" ondragstart="dragSearch(event)">
                    ...
                    <input type="text" id="ledgerSearch" aria-label="Search work orders" onkeyup="handleSearchInput(event)" ondblclick="resetSearchPosition()" placeholder="Search work orders... (Drag to column)" style="padding:8px 35px 8px 35px; border-radius:20px; border: 1px solid var(--text-accent); background:var(--input-bg); color:var(--text-primary); transition: all 0.3s; box-sizing: border-box;">
            ...
            <tr class="parent-row" data-id="<?= htmlspecialchars($i['wo_id']) ?>" style="<?= $rowStyle ?>">
                ...
                <td>
                    <?php
                        $stat = $i['status'];
                        if ($is_overdue) {
                            echo "<span class='status-escalated'>OVERDUE</span>";
                        } elseif ($stat === 'Completed') {
                            echo "<span class='status-closed'>$stat</span>";
                        } elseif ($stat === 'Cancelled' || $stat === 'Missed') {
                            echo "<span class='status-open'>$stat</span>";
                        } else {
                    ...
                    <div class="child-content">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
                            ...
                            <?php if ($i['status'] === 'Scheduled' || $i['status'] === 'In Progress' || $i['status'] === 'Overdue'): ?>
                                <a href="/_maint/wo_takeover.php?wo_id=<?= urlencode($i['wo_id']) ?>" class="btn btn-primary btn-sm">🚀 Takeover WO</a>
                            <?php endif; ?>
                        </div>
                        <?php 
                                ...
                                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                                    <?php foreach($parts as $part): ?>
                                        <div style="background: rgba(0,0,0,0.2); border: 1px solid var(--panel-border); padding: 5px 10px; border-radius: 6px; font-size: 0.85em; color: var(--text-secondary);">
                                            📦 <?= htmlspecialchars($parts_map[$part['part_id']] ?? 'Unknown Part') ?><?php if ($part['qty'] > 1): ?>
                                ...
                                <div style="display: flex; flex-direction: column; gap: 8px;">
                                    <?php foreach($chk_data as $idx => $chk): ?>
                                        <div style="background: rgba(0,0,0,0.2); border: 1px solid var(--panel-border); padding: 8px 12px; border-radius: 6px; font-size: 0.9em; color: var(--text-primary); display: flex; justify-content: space-between; align-items: center;">
                                            <span>✔️ <?= htmlspecialchars($chk['task_desc'] ?? '') ?> <small style="color: var(--text-muted);">(<?= $chk['expected_time_mins'] ?? 0 ?> mins)</small></span>
                                            <?php 
                                            $paths = [];
                                            ...
                                            ?>
                                                ...
                                                <?php foreach($paths as $pNum => $path): ?>
                                                    <button type="button" onclick="openImageOverlay('<?= htmlspecialchars($path) ?>')" style="background: var(--info-bg); border: 1px solid var(--info-border); color: var(--info); padding: 4px 10px; border-radius: var(--radius-sm); cursor: pointer; display: flex; align-items: center; gap: 5px; font-family: inherit;">📸 View Photo <?= count($paths) > 1 ? ($pNum+1) : '' ?></button>
                                                <?php endforeach; ?>
                                                </div>
                                            <?php endif; ?>
                                        </div>
