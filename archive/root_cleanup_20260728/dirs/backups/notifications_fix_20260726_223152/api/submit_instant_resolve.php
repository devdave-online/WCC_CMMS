...
try {
    ...
    // 3. Notify history / stats roles (same as formal closeout)
    require_once __DIR__ . '/../inc/notifications.php';
    $msg = 'Quick resolve closed: ' . $ticket_id . ' (by ' . $tech_name . ')';
    $actor = (int)($_SESSION['user_id'] ?? 0);
    wcc_notify_perm('view_history', 'ticket_closed', $msg, '/_rpt/history.php', 'success', $actor);
    wcc_notify_perm('view_statistics', 'ticket_closed', $msg, '/_rpt/history.php', 'success', $actor);

    echo json_encode(['status' => 'success', 'message' => 'Instant Fix logged successfully!', 'ticket_id' => $ticket_id]);
