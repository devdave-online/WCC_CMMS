...
<?php
$page_title = 'Technician Takeover';
require_once __DIR__ . '/../inc/head.php';
include __DIR__ . '/../nav.php';
?>

<div class="form-container">
    <div class="page-header"><h1>Log Intervention</h1><a href="../index.php" class="nav-btn">🔙 Cancel</a></div>
    <div class="ticket-info">
        ...
        <div style="margin-top:5px; color:var(--danger); font-weight:600;">Issue: <?= htmlspecialchars($ticket['fault_desc']) ?></div>
    </div>
    <input type="hidden" id="ticket_id" value="<?= htmlspecialchars($ticket['ticket_id']) ?>">
    
    <label for="tech_name">Overtook by:</label>
    <input type="text" id="tech_name" value="<?= htmlspecialchars($_SESSION['username'] ?? 'Unknown User') ?>" readonly>

    <div class="grid-2">
        <div><label for="action_start">Start Time:</label><input type="datetime-local" id="action_start" required></div>
        <div><label for="action_end">End Time:</label><input type="datetime-local" id="action_end" required></div>
    </div>
    ...
    <div class="grid-2">
        ...
        <div>
            <label for="escalated_to">Escalate To (Name):</label>
    ...
    <label for="root_cause">Root Cause of Breakdown:</label>
    ...
    <label for="parts_used">Parts Used (Optional):</label>
    <input type="text" id="parts_used" list="parts_list" placeholder="Search by part name or code... (Leave blank if none)">
