...
try {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'save_user') {
        ...
        if (isset($_POST['new_manual_skills']) && is_array($_POST['new_manual_skills'])) {
            ...
            foreach ($_POST['new_manual_skills'] as $sk) {
                if (trim($sk) !== '') {
                    $ins->execute([$edit_uid, trim($sk)]);
                }
            }
        }
    ...
    $manualSkills = [];
    try {
        $msStmt = $pdo->query("SELECT u.username, us.skill_name FROM user_skills us JOIN users u ON u.user_id = us.user_id");
        foreach($msStmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
            $manualSkills[$row['username']][] = $row['skill_name'];
        }
    } catch (Exception $e) {}

    // Helper for gamification level
    function getGamifiedLevel($hours) {
        if ($hours >= 200) return ['tier' => 'Master', 'icon' => '👑'];
        if ($hours >= 100) return ['tier' => 'Expert', 'icon' => '💎'];
        if ($hours >= 40) return ['tier' => 'Proficient', 'icon' => '🥇'];
        if ($hours >= 20) return ['tier' => 'Competent', 'icon' => '🥈'];
        if ($hours >= 10) return ['tier' => 'Advanced', 'icon' => '🥉'];
        return ['tier' => 'Novice', 'icon' => '🌱'];
    }
...
<div class="dashboard-container">
    ...
    <div style="display:flex; gap:10px; margin-bottom:15px; flex-wrap:wrap;">
        ...
        <button class="btn" onclick="showRegistrationConfigurator()">Registration Configurator</button>
        <button class="btn" onclick="showRolePresets()">Role Presets</button>
        <button class="btn" onclick="exportUsers()">Export CSV</button>
        <button class="btn" onclick="document.getElementById('skillConfigModal').style.display='block'">Skill Configurator</button>
    </div>
        ...
        <table class="data-table" id="usersTable">
            <thead>
                <tr>
                    ...
                    <th ondrop="dropSearch(event, this)" ondragover="allowDrop(event)">Skills</th>
            ...
            <tr class="parent-row" data-id="<?= $u['user_id'] ?>">
                ...
                <td style="white-space:nowrap;">
                    <?php
                        ...
                        $userGamified = $gamifiedStats[$u['full_name']] ?? [];
                        $gTokens = [];
                        foreach ($skillAutoConfigs as $sconf) {
                            ...
                            if (isset($userGamified[$cat]) && $userGamified[$cat] > 0) {
                                ...
                                $gTokens[] = "<span style='background:var(--panel-bg); color:var(--text-accent); border-radius:8px; padding:4px 8px; font-size:0.9em; display:inline-block; border: 1px solid var(--text-accent);'>{$sconf['icon']} " . htmlspecialchars($sconf['skill_name']) . " {$lvl['icon']} <span style='color:var(--text-secondary);'>(" . round($hrs, 1) . "h)</span></span>";
                            }
                        }
                        $gCount = count($gTokens);
                    ?>
                    <div style="display:flex; gap:4px; align-items:center;">
                        <button type="button" class="btn" style="padding:4px 8px; font-size:0.8em;" onclick="showUserSkills('Manual Skills (<?= htmlspecialchars(addslashes($u['username'])) ?>)', 'manual-skills-<?= $u['user_id'] ?>'); event.stopPropagation();" title="Manual Skills">🛠️ <?= $mCount ?></button>
                        <button type="button" class="btn" style="padding:4px 8px; font-size:0.8em;" onclick="showUserSkills('Gamified Skills (<?= htmlspecialchars(addslashes($u['username'])) ?>)', 'gamified-skills-<?= $u['user_id'] ?>'); event.stopPropagation();" title="Gamified Skills">🏆 <?= $gCount ?></button>
                    </div>
                    ...
                    <div id="manual-skills-<?= $u['user_id'] ?>" style="display:none;">
                        ...
                        if ($mCount > 0) {
                            foreach ($userManualSkills as $sk) {
                                echo "<div style='margin-bottom:8px;'><span style='background:rgba(255,255,255,0.1); border-radius:8px; padding:4px 8px; font-size:0.9em; display:inline-block; border: 1px solid rgba(255,255,255,0.2);'>🛠️ " . htmlspecialchars($sk) . "</span></div>";
                            }
                        } else {
                ...
                <td colspan="9">
                    <div class="child-content" style="display: grid; grid-template-columns: 1fr; gap: 20px;">
                        ...
                        <form method="POST" style="margin:0;">
                            ...
                            <?php 
                            $user_stats = $gamifiedStats[$u['username']] ?? [];
                            ...
                            <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; margin-bottom:15px;">
                                ...
                                <div style="padding:10px; background:var(--panel-bg); border-radius:8px; border:1px solid var(--panel-border);">
                                    ...
                                    <div style="display:flex; flex-wrap:wrap; gap:10px;">
                                        <?php if (!empty($user_stats)): ?>
                                            <?php foreach ($user_stats as $eq_type => $hrs): 
                                                if ($hrs > 0) {
                                                    $level = 'Novice';
                                                    $badge_color = '#94a3b8'; // gray
                                                    $icon = '🔧'; // default icon
                                                    if ($hrs >= 200) { $level = 'Master'; $badge_color = '#eab308'; $icon = '👑'; }
                                                    elseif ($hrs >= 100) { $level = 'Expert'; $badge_color = '#a855f7'; $icon = '💎'; }
                                                    elseif ($hrs >= 40) { $level = 'Proficient'; $badge_color = '#3b82f6'; $icon = '🥇'; }
                                                    elseif ($hrs >= 20) { $level = 'Competent'; $badge_color = '#10b981'; $icon = '🥈'; }
                                                    elseif ($hrs >= 10) { $level = 'Advanced'; $badge_color = '#f97316'; $icon = '🥉'; }
                                                    ...
                                                    echo '<div><strong style="color:var(--text-primary); font-size:1.1em;">'.htmlspecialchars($eq_type).'</strong><br>';
                                ...
                                <div style="padding:10px; background:var(--panel-bg); border-radius:8px; border:1px solid var(--panel-border); position:relative;">
                                    ...
                                    <div style="margin-top:10px;">
                                        <button type="button" class="nav-btn" style="padding:4px 8px; font-size:0.8em;" onclick="addNewSkillInput(<?= $u['user_id'] ?>)">+ Add New Skill</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 15px; gap: 8px; flex-wrap: wrap;">
                                ...
                                <button type="button" class="nav-btn" onclick="openWccConfirm('Reset permissions to role defaults?', function() { resetPerms(<?= $u['user_id'] ?>); }, 'Reset Perms');" style="font-size: 0.85em;">Reset Perms to Role</button>
                                <?php if(can('reset_passwords')): ?>
                                <button type="button" class="nav-btn secondary" style="font-size: 0.85em; border: 1px solid #ef4444; color: #ef4444; background: transparent;" onclick="openWccConfirm('Are you sure you want to reset this users password to the default password?', function() { resetPassword(<?= $u['user_id'] ?>); }, 'Reset Password');">Reset Password</button>
                                <?php endif; ?>
                            </div>
                        </form>
                    </div>
  ...
  <div class="modal-content" style="max-width: 560px; width: 100%; padding: 22px 24px;">
    ...
    <form method="POST">
      ...
      <div class="config-actions" style="margin-top:16px; display:flex; gap:10px; justify-content:flex-end;">
        <button type="submit" class="nav-btn primary" style="padding:9px 18px;">Save Configuration</button>
        <button type="button" class="nav-btn" onclick="document.getElementById('configModal').style.display='none'" style="padding:9px 18px;">Cancel</button>
      </div>
    </form>
  </div>
</div>
  ...
  <div class="modal-content" style="max-width: 860px; width: 96%; padding: 18px 22px; max-height: 92vh; overflow-y: auto;">
    ...
    <form method="POST">
      ...
      <div style="margin-top:14px; display:flex; gap:10px; justify-content:flex-end; align-items:center; flex-wrap:wrap;">
        ...
        <button type="submit" class="nav-btn primary" style="padding:8px 16px;">Save All Presets</button>
        <button type="button" class="nav-btn" onclick="document.getElementById('rolePresetsModal').style.display='none'" style="padding:8px 16px;">Cancel</button>
      </div>
    </form>
  </div>
</div>
...
<div id="skillConfigModal" class="modal">
  <div class="modal-content" style="max-width: 600px; width: 96%; padding: 18px 22px;">
    ...
    <p style="color:var(--text-secondary); font-size:0.85em; margin-bottom:15px;">
      Map Equipment Categories to specific gamified skills. When technicians log hours on these categories, they automatically level up!
    </p>
    ...
    <form method="POST">
      ...
      <div style="background:var(--panel-bg); border:1px solid var(--panel-border); padding:10px; border-radius:8px;">
        ...
        <button type="submit" class="btn primary" style="margin-top:10px;">Save Mapping</button>
      </div>
    </form>
  </div>
</div>
