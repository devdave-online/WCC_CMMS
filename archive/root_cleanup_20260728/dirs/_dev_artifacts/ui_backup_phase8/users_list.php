...
try {
    ...
    function getGamifiedLevel($hours) {
        if ($hours >= 200) return ['tier' => 'Master', 'icon' => '👑', 'color' => '#ef4444'];
        if ($hours >= 100) return ['tier' => 'Expert', 'icon' => '💎', 'color' => '#a855f7'];
        if ($hours >= 40) return ['tier' => 'Proficient', 'icon' => '🥇', 'color' => '#eab308'];
        if ($hours >= 20) return ['tier' => 'Competent', 'icon' => '🥈', 'color' => '#94a3b8'];
        if ($hours >= 10) return ['tier' => 'Advanced', 'icon' => '🥉', 'color' => '#d97706'];
        return ['tier' => 'Novice', 'icon' => '🌱', 'color' => '#cd7f32'];
    }
                    ...
                    <div class="child-content">
                        ...
                        <div style="margin-bottom:15px; padding:10px; background:var(--panel-bg); border-radius:8px; border:1px solid var(--panel-border);">
                            ...
                            <div style="display:flex; flex-wrap:wrap; gap:10px;">
                                <?php 
                                $user_stats = $gamifiedStats[$i['full_name']] ?? $gamifiedStats[$i['username']] ?? []; 
                                $hasGamified = false;
