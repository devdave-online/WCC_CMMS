...
function handle_work_orders($method, $id, $input) {
    ...
    switch ($method) {
        case 'GET':
            ...
            if ($id) {
                $stmt = $pdo->prepare("SELECT wo.*, e.equip_name FROM work_orders wo LEFT JOIN equipment e ON wo.equip_id = e.equip_id WHERE wo.wo_id = ?");
                $stmt->execute([$id]);
                ...
                $sql = "SELECT wo.*, e.equip_name FROM work_orders wo LEFT JOIN equipment e ON wo.equip_id = e.equip_id WHERE 1=1";
                $params = [];
        ...
        case 'POST':
            ...
            $stmt = $pdo->prepare("INSERT INTO work_orders (equip_id, title, description, status, scheduled_date, assigned_to, priority) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $input['equip_id'],
